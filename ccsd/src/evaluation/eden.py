...
def annotate(graphs, estimator=None, reweight=1.0, vertex_features=False, **opts):
    """Return graphs with extra node attributes: importance and features."""
    result = Vectorizer(**opts).annotate(
        graphs, estimator=estimator, reweight=reweight, vertex_features=vertex_features
    )
    return list(result)
...
class Vectorizer(AbstractVectorizer):
    ...
    def _graph_preprocessing(self, original_graph):
        ...
        _label_preprocessing(graph, key_label=self.key_label, bitmask=self.bitmask)
                    ...
                    for vertex_u in node_set:
                        if graph.nodes[vertex_u].get("node", False):
                            self._transform_vertex_pair(
                                graph, vertex_v, vertex_u, distance, feature_list
                            )
            ...
            for distance in range(self.min_d * 2, (self.d + 1) * 2, 2):
                if distance in root_dist_dict:
                    ...
                    for vertex_u in node_set:
                        if graph.nodes[vertex_u].get("node", False):
                            self._transform_vertex_pair(
                                graph, vertex_v, vertex_u, distance, node_feature_list
                            )
    ...
    def _transform_vertex_pair(
        self, graph, vertex_v, vertex_u, distance, feature_list, connection_weight=1
    ):
        ...
        for radius in range(self.min_r * 2, (self.r + 1) * 2, 2):
            ...
            if (
                self.weights_dict is None
                or self.weights_dict.get(radius_dist_key, 0) != 0
            ):
                self._transform_vertex_pair_valid(
                    graph,
                    vertex_v,
                    vertex_u,
                    radius,
    ...
    def _transform_vertex_pair_valid(
        self,
        graph,
        vertex_v,
        vertex_u,
        ...
        if radius < len_v and radius < len_u:
            ...
            half_feature = fast_hash_3(vertex_u_hash, radius, distance, self.bitmask)
    ...
    def _compute_neighborhood_graph_hash(self, root, graph):
        ...
        for node_set in root_dist_dict.keys():
            ...
            for v in root_dist_dict[node_set]:
                ...
                if self.positional:
                    vhlabel = fast_hash_2(graph.nodes[v]["hlabel"], root - v)
                else:
                    vhlabel = fast_hash_2(graph.nodes[v]["hlabel"], len(graph[v]))
                hash_label_list.append(vhlabel)
            # sort it
            hash_label_list.sort()
            # hash it
            hashed_nodes_at_distance_d_in_neighborhood = fast_hash(hash_label_list)
    ...
    def _compute_neighborhood_graph_weight(self, root, graph):
        ...
        for dist in root_dist_dict.keys():
            ...
            weight_array_at_d = np.array(
                [graph.nodes[v][self.key_weight] for v in root_dist_dict[dist]],
                dtype=np.float64,
            )
            if dist % 2 == 0:  # nodes
        ...
        graph.nodes[root]["neigh_graph_weight"] = neighborhood_graph_weight_list
    ...
    def _annotate_importance(self, graph, data_matrix):
        predictions, margins = self._compute_predictions_and_margins(graph, data_matrix)
            ...
            if graph.nodes[v].get("node", False):
                ...
                if self.key_weight in graph.nodes[v] and self.reweight != 0:
                    graph.nodes[v][self.key_weight] = (
                        self.reweight * abs(margins[vertex_id])
                        + (1 - self.reweight) * graph.nodes[v][self.key_weight]
                    )
...
def _label_preprocessing(graph, key_label="label", bitmask=2**20 - 1):
    for n in graph.nodes():
        ...
        for n in original_graph.nodes():
            if original_graph.nodes[n].get("edge", False):
                ...
                if len(endpoints) != 2:
                    ...
                    raise Exception(
                        "ERROR: expecting 2 endpoints in a single edge, instead got: %s"
                        % endpoints
                    )
                u = endpoints[0]
                v = endpoints[1]
