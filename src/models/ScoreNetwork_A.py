...
class ScoreNetworkA_CC(BaselineNetwork):
    """ScoreNetworkA_CC to calculate the score with respect to the adjacency matrix A."""
    ...
    def forward(
        ...
        for k in range(self.num_layers):
            x, adjc = self.layers[k](x, adjc, flags)
            adj_list.append(adjc)
        # Apply all the HodgeAdjLayer layers
        hodge_adj_list = [hodge_adjc]
        for k in range(self.num_layers):
            hodge_adjc = self.layers_hodge[k](hodge_adjc, rank2, flags)
            hodge_adj_list.append(hodge_adjc)

        # Concatenate the output of the AttentionLayer layers (B x N x N x (c_init + c_hid * (num_layers - 1) + c_final)
        adjs = torch.cat(adj_list, dim=1).permute(0, 2, 3, 1)
        out_shape = adjs.shape[:-1]  # B x N x N
        # Concatenate the output of the HodgeAdjLayer layers (B x (NC2) x (NC2) x (c_init + c_hid * (num_layers - 1) + c_final)
        hodge_adjs = torch.cat(hodge_adj_list, dim=1).permute(0, 2, 3, 1)
        adj_hodge_adjs = hodgedual_to_adj(
            hodge_adjs
